...
export async function POST(request: Request) {
  try {
    ...
    // 6. Process referral commissions if user signed up with a referral code
    if (referralCode) {
      try {
        // Import and use the referral service to process the 3-level chain
        const { referralService } = await import('@/lib/referral-service');
        await referralService.processReferral(referralCode, userUid);
        console.log(`Processed referral for user ${userUid} with code ${referralCode}`);
        
        // Trigger commission processing for sign-up event
        const { enhancedReferralCommissionService } = await import('@/lib/enhanced-referral-commission-service');
        const { CommissionEventType } = await import('@/config/referral-commission-config');
        
        const commissionResult = await enhancedReferralCommissionService.processCommissions(
          userUid,
          CommissionEventType.USER_SIGNUP,
          tierConfig.securityFee, // Commission based on security fee paid
          `signup_${userUid}_${Date.now()}` // Unique event ID for idempotency
        );
        
        if (commissionResult.success) {
          console.log(`Successfully processed ${commissionResult.commissionsProcessed} commissions totaling R${commissionResult.totalAmount}`);
        } else {
          console.warn(`Commission processing completed with issues: ${commissionResult.error}`);
        }
      } catch (referralError) {
        console.error('Error processing referral commissions:', referralError);
        // Don't fail signup if commission processing fails - log for manual review
      }
    }

    // 7. Clean up the pending user document after successful creation
    await db.collection('pending_signups').doc(temporaryId).delete();
