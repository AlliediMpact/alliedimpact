...
class NotificationService {
  ...
  subscribeToNotifications(userId: string, callback: (notifications: Notification[]) => void, options: {
    ...
    // Note: Query order must match Firestore index structure
    let constraints: any[] = [
      where('userId', '==', userId)
    ];
    ...
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        // Only log as error if it's not a permission issue (expected during logout)
        if (err.code === 'permission-denied') {
          console.debug('Notification subscription ended (permission denied):', err.message);
        } else {
