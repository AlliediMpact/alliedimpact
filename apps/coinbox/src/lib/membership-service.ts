...
class MembershipService {
  ...
  async upgradeMembership(userId: string, newTierId: string) {
    ...
    if (!userMembership) {
      ...
      // Process referral commissions for upgrade
      try {
        const { enhancedReferralCommissionService } = await import('./enhanced-referral-commission-service');
        ...
        const commissionResult = await enhancedReferralCommissionService.processCommissions(
          ...
          `upgrade_${userId}_${newTierId}_${Date.now()}` // Unique event ID for idempotency
        );
        
        if (commissionResult.success) {
          console.log(`Processed ${commissionResult.commissionsProcessed} upgrade commissions totaling R${commissionResult.totalAmount}`);
        }
      } catch (commissionError) {
        console.error('Error processing upgrade commissions:', commissionError);
        // Don't fail upgrade if commission processing fails
      }
