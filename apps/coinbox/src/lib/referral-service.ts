...
class ReferralService {
  ...
  async processReferral(referralCode: string, newUserId: string): Promise<void> {
    try {
      ...
      // Get Level 2 and Level 3 parents for 3-level commission tracking
      const referrerDoc = await getDoc(doc(db, 'users', referrerId));
      let level2ParentId: string | undefined;
      let level3ParentId: string | undefined;
      
      if (referrerDoc.exists()) {
        const referrerData = referrerDoc.data();
        level2ParentId = referrerData.referrerId || undefined;
        
        // Get Level 3 parent (Level 2's referrer)
        if (level2ParentId) {
          const level2Doc = await getDoc(doc(db, 'users', level2ParentId));
          if (level2Doc.exists()) {
            level3ParentId = level2Doc.data().referrerId || undefined;
          }
        }
      }

      // Add referral to user's record with 3-level chain
