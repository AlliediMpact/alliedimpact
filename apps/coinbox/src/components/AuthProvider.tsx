  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        const userDocRef = doc(db, 'flagged_users', currentUser.uid);
        const unsubscribeFlagged = firestoreOnSnapshot(userDocRef, (docSnapshot: FirestoreDocumentSnapshot) => { // Use aliased import
          if (docSnapshot.exists()) {
            ...
            signOutUser();
             ...
             // Silently handle permission errors for non-admin users (expected)
             console.debug('Flagged user status check (expected for regular users):', error.message);
        });
         ...
         try {
            ...
            if (userProfileSnap.exists()) {
                ...
                // User might be mid-signup, this is expected
                console.debug(`User document not yet created for UID: ${currentUser.uid} (may be completing signup)`);
            }
         } catch (error) {
