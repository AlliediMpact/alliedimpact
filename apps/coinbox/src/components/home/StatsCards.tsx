...
export default function StatsCards() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Try to fetch public stats first
        try {
          const publicStatsRef = collection(db, 'publicStats');
          const statsQuery = query(publicStatsRef, limit(1));
          const statsSnapshot = await getDocs(statsQuery);
          if (!statsSnapshot.empty) {
            const statsData = statsSnapshot.docs[0].data();
            setActiveUsers({
              total: statsData.totalUsers || 1000,
              recentUsers: statsData.recentUserNames || ['John', 'Sarah', 'Michael', 'Emma', 'David']
            });
          } else {
            throw new Error('No public stats available');
          }
        } catch (statsError) {
