...
0 2 * * * /path/to/coinbox/scripts/backup-firestore.sh >> /path/to/backup.log 2>&1
```
...
npm install -g pm2
pm2 start npm --name "coinbox" -- start
pm2 save
pm2 startup
```
...
# Docker
docker stop coinbox
docker run -d --name coinbox <previous-image-tag>
```
