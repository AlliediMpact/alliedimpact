...
const RATE_LIMITS = {
  ...
  voting: { requests: 20, windowMs: 60000 }, // 20 votes per minute
...
/**
 * SportsHub Middleware
 * Provides rate limiting and security for sports voting and payment operations
 */
export function middleware(request: NextRequest) {
  ...
  } else if (pathname.includes('/api/vote') || pathname.includes('/api/predict')) {
    limit = RATE_LIMITS.voting;
  } else if (pathname.includes('/api/payment') || pathname.includes('/api/payfast')) {
